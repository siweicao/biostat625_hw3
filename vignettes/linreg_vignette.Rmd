...
To compare the efficiency of `linreg()` and the `lm()` functions, we can use the ``

```{r}
bench::mark(linreg(Temp~Wind+Month+Ozone,airquality),lm(Temp~Wind+Month+Ozone,airquality))
```
